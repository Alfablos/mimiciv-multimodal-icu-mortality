...
def grad_cam(
    ...
    if not len(image_tensor) == len(tab_tensor):
        raise ValueError("ERROR: GradCAM function needs images, tabs to have the same size")
    if not len(image_tensor) == 1:
        raise ValueError('the grad_cam function only accepts 1 as the first dimension of the images tensor. This is a bug.')
    ...
    heat = F.interpolate(heat, size=(512, 512), mode='bilinear', align_corners=False) # no transpose afterwards!
