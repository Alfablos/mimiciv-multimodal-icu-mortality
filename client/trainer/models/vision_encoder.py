...
class Xencoder(nn.Module):
    def __init__(self, encoding_vector_dims, frozen_backbone, dropout):
        ...
        self.backbone: DenseNet = visionmodels.densenet121(
            weights=visionmodels.DenseNet121_Weights.DEFAULT
        )

        if frozen_backbone is None:
            raise ValueError(
                "A value for `frozen_backbone` MUST be specified for the X-Ray encoder."
            )

        if frozen_backbone:
            for name, param in self.backbone.named_parameters():
                if (
                    "denseblock4" not in name
                    and "norm5" not in name
        ...
        self.head = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.Linear(
                in_features=512, out_features=encoding_vector_dims
            ),  # encoding vector
        )
